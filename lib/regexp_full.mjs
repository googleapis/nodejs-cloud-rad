...
export const CLASS_REGEXP = new RegExp('^' + CLASS_MATCHER + '$');

// Like `{@link foo}` or `{@link foo|bar}` or `{@link foo | bar}`
export const INLINE_LINK_REGEXP = /\{@link\s+(\S+?)(?:\s*\|\s*(\S+))?\s*\}/g;

// Like `IBuild` or `INewInstanceRequest`
export const INTERFACE_REGEXP = new RegExp('^I' + CLASS_MATCHER + '$');
